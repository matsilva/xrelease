...
describe('setupGitHooks', { timeout: 20000 }, () => {
  beforeEach(async () => {
    vi.resetAllMocks();
    ...
    await setupTemplates({ workflow: true, changelog: true, hooks: true }, TEMPLATES, TEST_DIR);
  });
  ...
  describe('setupGitHooks', () => {
    it('should setup git hooks successfully', async () => {
      ...
      await expect(fs.access(path.join(TEST_DIR, '.husky'))).resolves.toBeUndefined();
      ...
      const commitlintConfig = await fs.readFile(path.join(TEST_DIR, 'commitlint.config.js'), 'utf-8');
      expect(commitlintConfig).toBe("export default { extends: ['@commitlint/config-conventional'] };");
    });
  });
});
