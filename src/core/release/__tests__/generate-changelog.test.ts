...
import { type BufferEncodingOption, execa, type Options } from "execa";
...
describe("generateChangelog", () => {
  ...
  it("should generate changelog with commits since last tag", async () => {
    ...
    vi.mocked(execa).mockImplementation(
      (_file: string, options?: Options<BufferEncodingOption>) => {
  ...
  it("should create new changelog if none exists", async () => {
    ...
    expect(fs.writeFile).toHaveBeenCalledWith(
      "CHANGELOG.md",
      expect.stringContaining("## [1.0.0] - 2024-01-01\n\n* initial commit\n\n")
    );
  });
  ...
  it("should handle no commits since last tag", async () => {
    ...
    expect(fs.writeFile).toHaveBeenCalledWith(
      "CHANGELOG.md",
      expect.stringContaining("## [1.0.1] - 2024-01-01\n\n* No changes\n\n")
    );
  });
