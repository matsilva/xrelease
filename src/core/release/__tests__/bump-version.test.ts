...
describe('bumpVersion', () => {
  ...
  it('should bump patch version correctly', async () => {
    ...
    expect(fs.writeFile).toHaveBeenCalledWith('package.json', expect.stringContaining('"version": "1.0.1"'));
  });
  ...
  it('should bump minor version correctly', async () => {
    ...
    expect(fs.writeFile).toHaveBeenCalledWith('package.json', expect.stringContaining('"version": "1.1.0"'));
  });
  ...
  it('should bump major version correctly', async () => {
    ...
    expect(fs.writeFile).toHaveBeenCalledWith('package.json', expect.stringContaining('"version": "2.0.0"'));
  });

  it('should throw if package.json has no version', async () => {
    vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify({}));

    await expect(bumpVersion()).rejects.toThrow('No version field found in package.json');
  });
