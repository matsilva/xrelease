...
import { type BumpType, bumpVersion } from "../../core/release/bump-version.js";
...
export async function createCommand(options: CreateOptions): Promise<void> {
  ...
  try {
    ...
    const newVersion = await bumpVersion(
      options.bump || config.release.defaultBump
    );
    ...
    if (config.release.version?.files?.length) {
      ...
      for (const file of config.release.version.files) {
        try {
          ...
          throw new Error(
            `Failed to update version in ${file.path}: ${error instanceof Error ? error.message : "Unknown error"}`
          );
        }
      }
    ...
    if (config.release.changelog?.enabled) {
      ...
      try {
        ...
        console.warn(
          chalk.yellow(
            "Could not read changelog file, proceeding without changelog content"
          )
        );
      }
        ...
        switch (action.type) {
          ...
          case "github-release": {
            ...
            if (!(await checkGitHubCLI())) {
              throw new Error(
                "GitHub CLI (gh) is required but not found. Please install it from: https://cli.github.com"
              );
            }
            ...
            if (action.assets) {
              ...
              const patterns = Array.isArray(action.assets)
                ? action.assets
                : [action.assets];
          ...
          default:
            if (action.command) {
              try {
                ...
                throw new Error(
                  `Custom action '${actionName}' failed: ${error instanceof Error ? error.message : "Unknown error"}`
                );
              }
            }
            break;
